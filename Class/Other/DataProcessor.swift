...
enum DataProcessor {
    ...
    func dataArrayWithFileName(_ aName: String) -> AnyObject {//use NSString is maybe have Multiple type
        switch self {//这里可以用 泛型 <T>，减少代码
        case .arrayWithPlistFileName: let optionArrayWithPlistFileName = {(plistFileName:String)-> NSArray in
            let plistPath = Bundle.main.path(forResource: plistFileName, ofType: nil)
